...
/**
 * The LivenessActivity is the entry point of the user liveness detection process.
 * <p/>
 * Make sure to add an instance of {@link LivenessTokenProvider} using {@link #EXTRA_TOKEN_PROVIDER} to the starting intent!
 * <p/>
 * The "resultCode" returned by this activity if started via {@link #startActivityForResult(Intent, int)} will be {@link #RESULT_OK}
 * (liveness detection successful) or {@link #RESULT_CANCELED} (liveness detection not successful).
 */
final public class LivenessActivity extends AppCompatActivity {
    /**
     * Key of the extra for the {@link LivenessTokenProvider}.
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LivenessTokenProvider tokenProvider = getIntent().getParcelableExtra(EXTRA_TOKEN_PROVIDER);
    ...
    private void setupFragment(@NonNull LivenessTokenProvider tokenProvider) {
        ...
        if (facialRecognitionFragment == null) {
            ...
            FacialRecognitionFragment fragment = FacialRecognitionFragment.newInstanceForLivenessDetection(tokenProvider);
